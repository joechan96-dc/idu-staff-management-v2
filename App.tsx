...
const initDPData = () => {
	...
	Object.values(YEAR_GROUPS).forEach((group) => {
		group.teachers.forEach((fullName) => {
			const code =
				Object.keys(TEACHER_NAME_MAP).find((key) => key === fullName) ||
				fullName.split(" ")[0];
	...
	lines.forEach((line) => {
		...
		let teacherFullName =
			Object.keys(TEACHER_NAME_MAP).find(
				(k) => TEACHER_NAME_MAP[k] === teacherCode
			) || // Find Full Name by Code
			Object.keys(TEACHER_NAME_MAP).find(
				(k) => TEACHER_NAME_MAP[k] === teacherNickname
			) ||
			teacherNickname;
		...
		if (
			yearGroup === "Year 12" ||
			yearGroup === "Year 13" ||
			classCode.startsWith("12") ||
			classCode.startsWith("13")
		) {
		...
		if (!DP_SCHEDULE[teacherFullName][dayKey])
			DP_SCHEDULE[teacherFullName][dayKey] = {};
...
const getLocationGroup = (loc: string) => {
	...
	if (
		loc.startsWith("LF") ||
		loc.includes("Lower Ground") ||
		loc.includes("LE1")
	)
		return "Lower Ground / 1st";
...
function App() {
	...
	const [selectedClass, setSelectedClass] = useState(
		YEAR_GROUPS[Object.keys(YEAR_GROUPS)[0]].classes[0]
	);
	...
	const handleDropTeacher = async (
		...
		if (isTeacherBusy(teacherName, day, block)) {
			alert(
				`${teacherName} is already scheduled in ${block} on ${day}! Cannot double book.`
			);
			return;
		}
	...
	const handleDropLocation = async (
		teacherName: string,
		location: string,
		day: string,
		block: string,
		...
		await saveAssignmentToSheet(
			...
			cls,
			currentTeachers,
			JSON.stringify(newSlotLocs)
		);
	};

	// --- Conflict Logic (DP) ---
	const getTeacherConflict = (
		teacherName: string,
		day: string,
		block: string
	) => {
		...
		if (
			classCode &&
			(classCode.startsWith("12") || classCode.startsWith("13"))
		) {
	...
	const displayedTeachers =
		viewMode === "byClass" && classMapping && classMapping[selectedClass]
							  ...
							  BLOCKS.map((block) => {
									...
									return (
										<ScheduleCard
											...
											year={selectedYear}
											cls={selectedClass}
											assignedTeachers={getAssignmentsForSlot(
												selectedDay,
												block,
												selectedYear,
												selectedClass
											...
											getConflict={(teacher) =>
												getTeacherConflict(teacher, selectedDay, block)
											}
										/>
									);
							  })
							  ...
							  yearData.classes.map((cls) => {
									...
									return (
										<ScheduleCard
											...
											getConflict={(teacher) =>
												getTeacherConflict(teacher, selectedDay, selectedBlock)
											}
										/>
									);
							  })}
						...
						<div className="p-3 overflow-y-auto custom-scrollbar flex-1">
							<div className="space-y-2">
								{displayedTeachers.map((teacher) => {
									...
									const isBusy =
										viewMode === "byBlock"
											? isTeacherBusy(teacher, selectedDay, selectedBlock)
											: false;
										...
										>
											<span
												className={`text-sm font-medium ${
													isBusy ? "text-gray-400" : "text-red-900"
												}`}
											>
												{teacher}
											</span>
									...
									<div key={groupName}>
										...
										<div className="flex flex-wrap gap-2">
											{locs.map((loc) => {
												...
												const isBusy =
													viewMode === "byBlock"
														? isLocationBusy(loc, selectedDay, selectedBlock)
														: false;
					...
					>
						<div
							className={`w-2.5 h-2.5 rounded-full ${
								getScriptUrl() ? "bg-emerald-500" : "bg-amber-500 animate-pulse"
							}`}
						></div>
						<div className="flex flex-col">
							<span className="text-sm font-bold">
								{getScriptUrl() ? "Ready to Sync" : "Setup Required"}
							</span>
							<span className="text-xs opacity-80">
								{getScriptUrl()
									? "Connected to Staffing Sheet"
									: "Connect to Google Sheet in Settings"}
							</span>
						</div>
					</div>
				</aside>
